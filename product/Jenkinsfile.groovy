...
def call() {
  ...
  pipeline {
    ...
    environment {
      GIT_URL = 'git@gitlab.com:jenkins1444027/product.git'
    ...
    stages {
      ...
      stage('Setting Dependencies Versions') {
        when {
          anyOf {
            branch 'release/*'
          }
        ...
        steps {
          script {
            println "---------------------   Setting Dependencies Versions Stage   ---------------------"
            withCredentials([usernamePassword(credentialsId: "${env.CREDENTIALS_ID}", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
              configFileProvider([configFile(fileId: "${env.SETTINGS_XML_ID}", variable: 'SETTINGS_XML_FILE')]) {
                ...
                println TELEMETRY_LST
                println ANALYTICS_LST

                sh """
                  mvn versions:set-property -Dproperty=telemetry.version -DnewVersion="${TELEMETRY_LST}" -s ${SETTINGS_XML_FILE} -Dusername=${USERNAME} -Dpassword=${PASSWORD}
                  mvn versions:set-property -Dproperty=analytics.version -DnewVersion="${ANALYTICS_LST}" -s ${SETTINGS_XML_FILE} -Dusername=${USERNAME} -Dpassword=${PASSWORD}
                  mvn dependency:list
                """
              }
            }
          ...
          script {
            ...
            withCredentials([usernamePassword(credentialsId: "${env.CREDENTIALS_ID}", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
              configFileProvider([configFile(fileId: "${env.SETTINGS_XML_ID}", variable: 'SETTINGS_XML_FILE')]) {
                ...
                // sh "java -cp simulator-${SIMULATOR_LST}.jar:telemetry-${TELEMETRY_LST}.jar:analytics-${ANALYTICS_LST}.jar com.lidar.simulation.Simulator"
...
def runE2ETestsPartly(part, simulator_lst, telemetry_lst, analytics_lst) {
